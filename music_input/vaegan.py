...
class vaegan(object):
    ...
    def __init__(self, batch_size, max_epoch, model_path, data,
        ...
        self.content_music = tf.placeholder(tf.float32, [self.batch_size, 2, 882000, 1])

        self.images = tf.placeholder(tf.float32, [self.batch_size, 2, 882000, 1])
        ...
        self.con_dataset = self.con_read('/home/yyan7582/Sample/SampleDataSet/01Happy_Birthday.wav')
    ...
    def build_model_infoGan(self):
        ...
        #for Gan generator
        self.gen_learning_rate = tf.train.exponential_decay(self.learning_rate_gen,self.global_train_step,200,0.95,staircase=True)
        self.Style_gen_learning_rate = tf.train.exponential_decay(self.learning_rate_gen, self.global_style_train_step, 100, 0.95,
                                                            staircase=True)

        self.x_p, self.con_dcov2, self.con_dcov3,self.con_dcov4= self.generate(self.con_cov3)
        _,_,_,_, self.reslt_encode = self.Encode(self.x_p, self.cov1,self.cov2,self.sty_encoder,True)        

        # for G Loss
        self.gen_lost = tf.reduce_mean(tf.square(self.content_music - self.x_p))
        self.gen_detail_lost1 = tf.reduce_mean(tf.square(self.sty_encoder - self.con_dcov2))
        self.gen_detail_lost2 = tf.reduce_mean(tf.square(self.cov2 - self.con_dcov3))
        self.gen_detail_lost3 = tf.reduce_mean(tf.square(self.cov1 - self.con_dcov4))
#         self.gen_detail_lost4 = tf.reduce_mean(tf.square(self.con_cov4 - self.con_dcov3))
#         self.gen_detail_lost5 = tf.reduce_mean(tf.square(self.con_cov5 - self.con_dcov2))
        ...
        self.l3_loss =  tf.reduce_mean(tf.square(self.sty_encoder-self.reslt_encode))
        ...
        #for Gen
        #self.G_fake_loss ++ self.l3_loss
        self.g_loss = 5*self.gen_lost  +self.sty_gen_detail_lost+self.gen_detail_lost1+self.gen_detail_lost2+self.gen_detail_lost3
    ...
    def con_train(self):
        ...
        opti_con_e = tf.train.RMSPropOptimizer(learning_rate=0.001).minimize(self.style_en_lost,
                                                                                                var_list=self.sty_e_vars)
        opti_con_G = tf.train.RMSPropOptimizer(learning_rate=self.Style_gen_learning_rate).minimize(self.style_gen_lost,
            ...
            while e <= self.con_ecoh:
                ...
                while batch_num < len(self.ds_train) / self.batch_size:
                    ...
                    if np.mod(step, 500) == 0:
                        ...
                        sample_audio = np.reshape(sample_audio,[-1,2])
                        ...
                        write("output_Style_generated_{:04d}.wav".format(step), 44100, sample_audio)
    ...
    def train(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            point = []
            tem = self.ds_train
            realbatch_array = tem[2]
            realbatch_array = np.reshape(realbatch_array, [-1, 2, 882000, 1])
            print("Start Training...")
...
#                     
                    ...
                    D_loss = sess.run(self.D_loss,
                                      feed_dict={self.images: realbatch_array, self.content_music: self.con_dataset})
                    print(D_loss,"-----D_lOSS")
                    ...
                    if np.mod(step , 1000) == 1:
                        ...
                        #                                                              n_iter=10)
                        # recovered_audio_orig = recovered_audio_orig * 10000000
                        write("output_generated_{:04d}.wav".format(step), 44100, sample_audio)
    ...
    def test(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for i in range(0, flag):
                ...
                input_style_data = np.reshape(input_style_data, [-1,2,882000,1])
                ...
                # recovered_audio_orig = recovered_audio_orig * 10000000
                output_music = np.reshape(output_music,[-1,2])

                print("Strat generate music")

                write("output_resultTest_generated_{:04d}.wav".format(i), 44100, output_music)
    ...
    def discriminate(self, x_var, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            conv1 = lrelu(conv2d(x_var, output_dim=2, name='dis_conv1'))
            conv2= lrelu(batch_normal(conv2d(conv1, output_dim=2 , name='dis_conv2'), scope='dis_bn1', reuse=reuse))
            conv3= lrelu(batch_normal(conv2d(conv2, output_dim=4 ,name='dis_conv3'), scope='dis_bn2', reuse=reuse))
            conv4= lrelu(batch_normal(conv2d(conv3, output_dim=4, name='dis_conv4',d_w=2), scope='dis_bn3', reuse=reuse))
            conv5 = lrelu(batch_normal(conv2d(conv4, output_dim=4, name='dis_conv5',d_w=5), scope='dis_bn4', reuse=reuse))
            middle_conv = conv5
            conv6 = tf.reshape(conv5, [self.batch_size, -1])
            ...
            output = fully_connect(conv6, output_size=1, scope='dis_fully2')
    ...
    def generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('generator') as scope:
            ...
            d3 = tf.nn.relu(batch_normal(de_conv(d2 , output_shape=[self.batch_size, 2, 220500, 4] , name='gen_deconv2',d_w=2), scope='gen_bn2', reuse=reuse))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[self.batch_size, 2, 441000, 2] , name='gen_deconv3',d_w=2), scope='gen_bn3', reuse=reuse))
            d5 = tf.nn.relu(batch_normal(de_conv(d4, output_shape=[self.batch_size, 2, 882000, 1], name='gen_deconv4',d_w=2), scope='gen_bn4', reuse=reuse))
            ...
            d8 = de_conv(d5, output_shape=[self.batch_size, 2, 882000, 1], name='gen_deconv5', d_h=1, d_w=1)
    ...
    def Encode(self, x, cov1,cov2,cov3,reuse=False):
        ...
        with tf.variable_scope('encode') as scope:
            ...
            conv1 = tf.nn.relu(batch_normal(conv2d(x, output_dim=2, name='e_c1',d_w=2), scope='e_bn1'))

            conv1 = conv1 + cov1
            print(np.shape(conv1))
            conv2 = tf.nn.relu(batch_normal(conv2d(conv1, output_dim=4, name='e_c2',d_w=2), scope='e_bn2'))
            conv2 = conv2 + cov2
            print(np.shape(conv2))

            conv3 = tf.nn.relu(batch_normal(conv2d(conv2 , output_dim=6, name='e_c3',d_w=2), scope='e_bn3'))
            conv3 = conv3 + cov3
            ...
            z_mean = batch_normal(fully_connect(conv6 , output_size=1, scope='e_f6'), scope='e_bn6')
            z_sigma = batch_normal(fully_connect(conv6, output_size=1, scope='e_f7'), scope='e_bn7')
    ...
    def Style_generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('sty_generator') as scope:
            ...
            d2 = tf.reshape(z_var, [self.batch_size, 2, 110250, 6])
            d2 = tf.nn.relu(batch_normal(de_conv(d2 , output_shape=[self.batch_size, 2, 220500, 4] , name='gen_deconv2',d_w=2), scope='gen_bn2'))
            d3 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[self.batch_size, 2, 441000, 2] , name='gen_deconv3',d_w=2), scope='gen_bn3'))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[self.batch_size, 2, 882000,1] , name='gen_deconv4',d_w=2),scope='gen_bn4', reuse=reuse))
            # d6 = tf.nn.relu(batch_normal(de_conv(d4, output_shape=[self.batch_size, 2, 176400, 2], name='gen_deconv6'),scope='gen_bn6', reuse=reuse))
            # d7 = tf.nn.relu(batch_normal(de_conv(d6, output_shape=[self.batch_size, 2, 882000, 1], name='gen_deconv7'),scope='gen_bn7', reuse=reuse))
            d8 = de_conv(d4, output_shape=[self.batch_size, 2, 882000, 1], name='gen_deconv5', d_h=1, d_w=1)
    ...
    def con_read(self, path):
        ...
        dataSet = np.reshape(x, [-1, 2, 882000, 1])
        print(np.shape(dataSet))
        return dataSet
