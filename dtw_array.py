...
def DTW(style_m, content, window=None, d=lambda x, y: tf.abs(tf.subtract(x, y))):
    ...
    content_array = tf.TensorArray(dtype=tf.float32,size=44100,flow=1)

    style_array = tf.TensorArray(dtype=tf.float32,size=44100,flow=1)
    ...
    content_array = content_array.scatter([0,44100],content)
    style_array = style_array.scatter([0,44100],style)
