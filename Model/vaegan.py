...
class vaegan(object):
    ...
    def generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('generator') as scope:
            ...
            d3 = tf.nn.relu(batch_normal(de_conv(d2 , output_shape=[self.batch_size, 430, 64, 128] , name='gen_deconv2',d_h=2), scope='gen_bn2', reuse=reuse))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[self.batch_size, 1720, 256,64] , name='gen_deconv3'), scope='gen_bn3', reuse=reuse))
    ...
    def Encode(self, x, cov1,cov2,cov3):
        ...
        with tf.variable_scope('encode') as scope:
            ...
            conv3 = tf.nn.relu(batch_normal(conv2d(conv2 , output_dim=256, name='e_c3',d_h=2), scope='e_bn3'))
            # conv3 = conv3 + cov3
    ...
    def Style_generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('sty_generator') as scope:
            ...
            d2 = tf.reshape(z_var, [self.batch_size, 215, 16, 256])
            d2 = tf.nn.relu(batch_normal(de_conv(d2 , output_shape=[self.batch_size, 430, 64, 128] , name='gen_deconv2',d_h=2), scope='gen_bn2'))
            d3 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[self.batch_size, 1720, 256,64] , name='gen_deconv3'), scope='gen_bn3'))
